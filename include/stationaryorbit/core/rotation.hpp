...
namespace zawa_ch::StationaryOrbit
{
	...
	struct Rotation
	{
		static_assert(IntegralTypeTraits::is_integraltype<Tp> && BitSequenceTypeTraits::is_bitsequence_type<Tp>, "テンプレート引数 Tp は、型要件:IntegralType と型要件:BitSequenceType をともに満たす必要があります。");
	private:
		Tp _value;
	...
	public:
		...
		template<typename valueT, std::enable_if_t<NumericalTypeTraits::is_numericaltype<valueT>, int> = 0>
		...
		template<typename valueT = long double, std::enable_if_t<NumericalTypeTraits::is_numericaltype<valueT>, int> = 0>

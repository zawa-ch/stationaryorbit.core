...
namespace zawa_ch::StationaryOrbit
{
	...
	{
		static_assert(SequencialOrderIteratorTraits::is_sequencial_order_iterator<T>, "テンプレート型 T は 型要件:SequencialOrderIterator を満たす必要があります。");
	public:
		typedef SequencialOrderIteratorTraits::ValueType<T> value_type;
		typedef SequencialOrderIteratorTraits::ValueType<T> reference;
		typedef const SequencialOrderIteratorTraits::ValueType<T> const_reference;
		typedef IteratorAdapter<T> iterator;
		typedef IteratorAdapter<T> const_iterator;
		typedef SequencialOrderIteratorTraits::IteratorDiff difference_type;
		typedef size_t size_type;
	private:
		T _itr;
	public:
		constexpr explicit IteratorAdaptContainer(const T& iterator) : _itr(iterator) {}
	...
	template<typename T> class IteratorReverseAdaptContainer;
	...
	public:
		...
		///	イテレータの方向を反転します。
		constexpr IteratorReverseAdaptContainer<T> reverse() const { return IteratorReverseAdaptContainer(_itr); }
